...
import BlossomInput from './BlossomInput';
...
export default function BlossomSolver() {
    ...
    const handleBlossomInputChange = useCallback((center: string, outer: string) => {
        ...
        <div>
            <div className="mb-8 max-w-3xl mx-auto">
                <div className="flex flex-col items-center gap-4 mb-6">
                    <div className="inline-flex p-1 bg-gray-200 rounded-full">
                        ...
                        <button
                            type="button"
                            className={`px-5 py-2.5 rounded-full transition-all duration-300 ${inputMode === 'blossom'
                                ? 'bg-white shadow text-gray-800 font-medium'
                                : 'text-gray-600 hover:text-gray-800'
                                }`}
                            onClick={() => setInputMode('blossom')}
                        >
                            Blossom Input
                        </button>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="transition-all duration-300">
                    {inputMode === 'text' ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
                                ...
                                <input
                                    ...
                                    className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-400 focus:border-transparent text-lg text-center uppercase font-medium"
                        ...
                        <div className="mb-6 transition-all duration-300">
                            <BlossomInput
                                onLettersChange={handleBlossomInputChange}
                    ...
                    <div className="mt-8 text-center flex items-center justify-center">
                        <div className="relative">
                            <button
                                type="submit"
                                disabled={loading}
                                className="px-8 py-3 bg-gradient-to-r from-pink-400 to-rose-500 text-white font-medium text-lg rounded-full hover:from-pink-500 hover:to-rose-600 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 disabled:opacity-50 transition-all duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                            >
                                {loading ? (
            ...
            {results && (
                <div id="results-section" className="results mt-12 transition-all duration-500 animate-fadeIn">
                    <div className="mb-8">
                        <div className="h-1 w-full bg-gradient-to-r from-transparent via-pink-300 to-transparent mb-8"></div>
                        <div className="flex flex-wrap -mx-2">
                            <div className="w-full md:w-1/3 px-2 mb-4">
                                <div className="bg-gradient-to-br from-pink-50 to-rose-100 rounded-xl p-4 border border-pink-200 shadow-sm h-full">
                                    <div className="text-center">
                                        <div className="text-4xl font-bold text-pink-600 mb-1">{results.totalCount}</div>
                                        <div className="text-pink-800 text-sm font-medium">Total Words</div>
                                    </div>
                                </div>
                            </div>
                            <div className="w-full md:w-1/3 px-2 mb-4">
                                <div className="bg-gradient-to-br from-pink-50 to-rose-100 rounded-xl p-4 border border-pink-200 shadow-sm h-full">
                                    <div className="text-center">
                                        <div className="text-4xl font-bold text-pink-600 mb-1">{results.pangrams.length}</div>
                                        <div className="text-pink-800 text-sm font-medium">Pangrams</div>
                                    </div>
                                </div>
                            </div>
                            <div className="w-full md:w-1/3 px-2 mb-4">
                                <div className="bg-gradient-to-br from-pink-50 to-rose-100 rounded-xl p-4 border border-pink-200 shadow-sm h-full">
                                    <div className="text-center">
                                        <div className="text-4xl font-bold text-pink-600 mb-1">{calculateTotalPoints()}</div>
                                        <div className="text-pink-800 text-sm font-medium">Estimated Points</div>
                                    </div>
                                </div>
                            </div>
                    ...
                    {results.pangrams.length > 0 && (
                        <div className="mb-8">
                            <h3 className="text-xl font-bold mb-4 text-gray-800 flex items-center">
                                <div className="w-8 h-8 bg-pink-400 rounded-full flex items-center justify-center mr-2 text-white">
                                    P
                                </div>
                            ...
                            <div className="bg-pink-50 p-4 rounded-lg border border-pink-200">
                                <div className="flex flex-wrap gap-2">
                                    {results.pangrams.map((word) => (
                                        <span
                                            key={word}
                                            className="inline-block px-4 py-2 bg-pink-200 rounded-full text-sm font-medium text-pink-800 shadow-sm transition-all hover:shadow hover:bg-pink-300"
                                        >
                                            {word.toUpperCase()}
                                        </span>
                                    ))}

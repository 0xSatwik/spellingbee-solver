  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-indigo-100 p-8">
      <div className="max-w-6xl mx-auto bg-white p-8 rounded-xl shadow-xl">
        <h1 className="text-4xl font-bold text-center mb-8 text-indigo-800">Spelling Bee Archive</h1>
        ...
        <div className="flex justify-center mb-8">
          <div className="flex space-x-2 bg-gray-100 p-1 rounded-lg">
            <button 
              ...
              className={`px-4 py-2 rounded-md transition-all ${activeTab === 'search' ? 'bg-indigo-600 text-white shadow-md' : 'hover:bg-gray-200'}`}
            >
              Search Archive
            </button>
            {currentPuzzle && (
              <button 
                ...
                className={`px-4 py-2 rounded-md transition-all ${activeTab === 'puzzle' ? 'bg-indigo-600 text-white shadow-md' : 'hover:bg-gray-200'}`}
              >
                Current Puzzle
              </button>
            )}
            {searchResults.length > 0 && (
              <button 
                ...
                className={`px-4 py-2 rounded-md transition-all ${activeTab === 'results' ? 'bg-indigo-600 text-white shadow-md' : 'hover:bg-gray-200'}`}
              >
                Search Results
              </button>
            )}
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 text-red-600 p-5 rounded-lg mb-6 shadow-sm flex items-start">
        ...
        {activeTab === 'search' && (
          <div className="mb-8 grid grid-cols-1 md:grid-cols-2 gap-8">
            {/* Puzzle ID Search */}
            <div className="p-6 bg-gradient-to-br from-indigo-50 to-blue-100 rounded-xl shadow-md transition-all hover:shadow-lg border border-indigo-100">
              <h2 className="text-2xl font-bold mb-4 text-indigo-800">Find by Puzzle ID</h2>
              <div className="mb-4">
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <input
                      ...
                      min="1"
                      className="w-full p-3 pl-8 border border-indigo-200 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-300 focus:border-transparent"
                  ...
                  <button
                    ...
                    className="bg-indigo-600 text-white py-2 px-6 rounded-lg hover:bg-indigo-700 transition-colors shadow-md hover:shadow-lg flex items-center"
            ...
            {/* Calendar Picker */}
            <div className="p-6 bg-gradient-to-br from-purple-50 to-pink-100 rounded-xl shadow-md transition-all hover:shadow-lg border border-purple-100">
              <h2 className="text-2xl font-bold mb-4 text-purple-800">Find by Date</h2>
              <div className="mb-4">
        ...
        {activeTab === 'results' && (
          <div className="mb-8">
            <h2 className="text-2xl font-bold mb-4 text-indigo-800">Search Results</h2>
            
            {searchResults.length > 0 ? (
              <>
                <p className="mb-6 text-gray-600">Found {searchResults.length} puzzles matching your search.</p>
                
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {searchResults.map((result, index) => (
                    <div key={index} className="border border-indigo-100 rounded-lg p-5 shadow-md hover:shadow-lg transition-all bg-gradient-to-r from-white to-indigo-50">
                      ...
                      <p className="mb-3"><span className="font-semibold">Pangrams:</span> {result.puzzle.pangrams_count}</p>
                      ...
                      <button
                        ...
                        className="text-sm bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition-colors shadow-sm hover:shadow-md"
                      >
                        View Details
                      </button>
                    </div>
              ...
              <div className="text-center py-10">
                <div className="inline-block p-6 bg-indigo-50 rounded-xl shadow-sm">
        ...
        {activeTab === 'puzzle' && currentPuzzle && (
          <div className="mb-8">
            <div className="text-center mb-8">
              ...
              <div className="inline-block px-6 py-3 bg-indigo-50 rounded-lg shadow-sm">
                <p className="mt-2">
            ...
            <div className="mt-10 bg-white p-6 rounded-xl shadow-md">
              <h3 className="text-2xl font-bold mb-5 text-indigo-800">Words</h3>
              
              {lengths.map(length => (
                <div key={length} className="mb-8">
                  <h4 className="text-lg font-semibold mb-3 pl-2 border-l-4 border-indigo-500">{length}-letter words ({wordsByLength[length].length})</h4>
                  <div className="flex flex-wrap gap-2">
                    {wordsByLength[length].map((word, index) => (
                      <div 
                        key={index}
                        className={`px-3 py-1 rounded-full shadow-sm ${word.is_pangram ? 'bg-yellow-300 font-semibold' : 'bg-gray-100'} hover:transform hover:scale-105 transition-transform`}
                      >
                        {word.word}
                      </div>
                    ))}
        ...
        <div className="flex justify-center mt-10 space-x-4">
          <Link 
            href="/" 
            className="px-5 py-2 bg-gradient-to-r from-indigo-600 to-blue-600 text-white rounded-lg hover:from-indigo-700 hover:to-blue-700 transition-all shadow-md hover:shadow-lg"
          >
            Home
          </Link>
          <Link 
            href="/today" 
            className="px-5 py-2 bg-gradient-to-r from-yellow-500 to-amber-500 text-white rounded-lg hover:from-yellow-600 hover:to-amber-600 transition-all shadow-md hover:shadow-lg"
          >
            Today's Puzzle
          </Link>
          <Link 
            href="/yesterday" 
            className="px-5 py-2 bg-gradient-to-r from-purple-500 to-violet-500 text-white rounded-lg hover:from-purple-600 hover:to-violet-600 transition-all shadow-md hover:shadow-lg"
          >
            Yesterday's Puzzle
          </Link>
          <Link 
            href="/stats" 
            className="px-5 py-2 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-lg hover:from-green-600 hover:to-emerald-600 transition-all shadow-md hover:shadow-lg"
          >
            Statistics
          </Link>
        </div>
